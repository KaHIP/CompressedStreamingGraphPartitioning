...
onepass_fennel_approx_sqrt::onepass_fennel_approx_sqrt(
    PartitionID k0, PartitionID kf, PartitionID max_blocks, NodeID n_threads,
...
float onepass_fennel_approx_sqrt::compute_score(floating_block &block,
                                                int my_thread) {
  return block.get_fennel_approx_sqrt_obj(my_thread);
}

void onepass_fennel_approx_sqrt::instantiate_blocks(LongNodeID n, LongEdgeID m,
                                                    PartitionID k,
